...
INSERT_PRACTICE_MR = "INSERT INTO best_practice_mr (best_practice, username, "\
                     "tg_id, datetime_added, desc, file_link, checked, " \
                     "kas_approved, cm_approved, active) " \
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
BP_KAS = "UPDATE best_practice_mr SET kas_approved = ?, checked = True WHERE id " \
         "= ?"
BP_CM = "UPDATE best_practice_mr SET cm_approved = ?, checked = True WHERE " \
        "id = ?"
